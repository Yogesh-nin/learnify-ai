...
export async function listCoursesForUser(email, { page, size, offset }) {
  const where = eq(STUDY_MATERIAL_TABLE.createdBy, email);
  ...
  const [courses, [{ total }]] = await Promise.all([
    db
      ...
      .limit(size)
      .offset(offset),
    db.select({ total: count() }).from(STUDY_MATERIAL_TABLE).where(where),
  ]);
